...
GM.vars = {
    ...
    GM.apis = {
        articleReply: "apis/minisite/article_reply.json",
        postReply: "apis/group/post_reply.json@retrieve_type=by_post",
        topAdImg: "apis/flowingboard/item/mguokr_top_ad.json",
        hotArticle: "apis/flowingboard/item/m_hot_content.json",
        maddenArticle: "apis/flowingboard/item/hot_madden_content.json"
    },
    GM.ajax = {
        articleReply: function(a, b, c) {
            var d = b || 0,
                e = c || 10,
                f = {
                    article_id: a,
                    ...
                    limit: e,
                    desc: "false",
                    retrieve_type: "by_article"
                };
            $.ajax({
                type: "get",
                url: GM.apis.articleReply,
                data: f,
                dataType: "json",
                cache: !0,
                conmplete: function(a) {
                    GM.vars.reply_total = a.total,
                        GM.render.articleReply(a),
                        GM.vars.reply_total > $("#replyList>article").length ? $("#loadMoreReply").find("span").show().prev().removeClass("loadMoreAnimate") : $("#loadMoreReply").hide(),
                        $(".cmt-main img").each(function() {
                    ...
                    var b = $(".comment"),
                        c = b.find("embed"),
                        d = b.find("iframe");
                    c.replaceWith("<p>（此处视频，暂不支持播放）</p>"),
                        d.replaceWith("<p>（此处视频，暂不支持播放）</p>")
                }
            })
        },
        postReply: function(a, b, c) {
            var d = b || 0,
                e = c || 10,
                f = {
                    post_id: a,
        ...
        loadHotArticle: function() {
            var a = Math.floor(74 * Math.random()),
                b = "",
                c = ""; - 1 !== document.cookie.indexOf("_is_search") ? (b = GM.apis.maddenArticle, c = "mobile_madden_content:", _gaq.push(["_trackEvent", "loadhot_from_search", window.location.href, "hotStuff"])) : (b = GM.apis.hotArticle, c = "mobile_hot_content:", _gaq.push(["_trackEvent", "load_hot", window.location.href, "hotStuff"])),
                "article" == GM.vars.GA_TYPE ? c += "article": "post" == GM.vars.GA_TYPE ? c += "post": "question" == GM.vars.GA_TYPE && (c += "question"),
                $.ajax({
                    type: "get",
                    url: b,
                    data: {
                    ...
                    success: function(a) {
                        ...
                        GM.render.hotArticleList(a, c)
                    }
                })
        }
    },
    GM.temp = {
        articleReply: '<article class="comment"><footer class="cmt-more"><div class="gclear"><address class="cmt-author gfl"><a href="#">{name}</a></address><time class="cmt-time gfr">{time}</time></div><div class="cmt-order">{num}楼</div></footer><p class="cmt-main">{content}</p></article>',
        hotArticleList: '<li><a href="{item_url}" data-gaevent="{ga_event}"><h4>{item_desc}</h4><div class="title-detail"><img src="{img_src}"><p>{item_summary}</p></div></a></li>',
        hotArticleListNotFirst: '<li><a href="{item_url}" data-gaevent="{ga_event}"><h4>{item_desc}</h4></a></li>',
    ...
    GM.render = {
        articleReply: function(a) {
            ...
            $.each(a.result,
                function(a) {
                    var d = GM.utils.dateFormat(this.date_created),
                        e = (this.author.avatar.normal, c + a + 1),
                        f = this.author.nickname,
                        g = this.html;
                    b += GM.temp.articleReply.replace("{time}", d).replace("{num}", e).replace("{content}", g).replace("{name}", f)
                }),
                $("#replyList").append(b)
        },
        hotArticleList: function(a, b) {
            var c = "",
                d = a.length,
                e = d > 5 ? 5 : d;
            if (d) for (var f = Math.max(e, d), g = !1, h = 0; f > h && e; h++) {
                var i = a[h],
                    j = i.title;
                i.title && i.url && (!g && i.image && i.summary ? (c = GM.temp.hotArticleList + c, c = c.replace(/\{item_url\}/gi, i.url).replace("{img_src}", i.image).replace("{item_desc}", j).replace("{item_summary}", i.summary).replace(/\{ga_event\}/gi, b), g = !0) : c += GM.temp.hotArticleListNotFirst.replace(/\{item_url\}/gi, i.url).replace("{item_desc}", j).replace(/\{ga_event\}/gi, b), --e),
                    $("#hotStuff").html(c)
            }
        }
    },
    GM.utils = {
        ...
        viewLarge: function(a) {
            ...
            var a = a,
                ...
                e = a.width(),
                f = a.height();
            if (d.toLowerCase().indexOf("guokr.com") > 0) {
    ...
    GM.init = {
        ...
        loadArticleReply: function(a) {
            GM.vars.article_reply_id = a,
                GM.vars.reply_type = "articleReply",
                GM.ajax.articleReply(a, 0, 10)
        },
        loadPostReply: function(a) {
            GM.vars.post_reply_id = a,
                GM.vars.reply_type = "postReply",
                GM.ajax.postReply(a, 0, 10)
        },
        ...
        getTopAd: function() {
            GM.vars.top_img_src && GM.vars.top_download_url
        },
...
_gaq.push(["_setAccount", "UA-19521615-1"]),
    _gaq.push(["_setDomainName", "guokr.com"]),
        ...
        $(document).ready(function() {
            $("#to-top").click(function() {
                ...
                $(".content-main").find("img").each(function() {
                    ...
                    a.indexOf("guokr.com") > 0 && (a.indexOf("thumbnail") > 0 || a.indexOf("gkimage") > 0) && $(this).wrap('<div class="ZoomBox"><div class="content-zoom ZoomIn"></div></div>').closest(".ZoomIn").append('<a href="javascript:">放大</a>')
                }),
