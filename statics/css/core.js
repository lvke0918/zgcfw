...
XPE.clip=function() {
 var _board={};
  return {
    copy: function(type, param ) {
        YAHOO.log("set board:"+param.objectId);
        _board=param;
        YAHOO.log("board set:"+ _board.objectId);
    }
    ,
    getBoard : function() {
        YAHOO.log("board is:"+_board.objectId);
      return _board;
    }
  }
}();

XPE.bus.subscribe('copy',XPE.clip.copy);

XPE.inited=false;

XPE.init=function() {
  if (XPE.inited) return;
  XPE.inited=true;
  XPE.bus.occurred('init_ui','global');
  XPE.bus.occurred('init','global');
};
    ...
    for(var i=0;i<selTemp.childNodes.length;i++){
        if(selTemp.childNodes[i].tagName){
            ...
            if (YAHOO.env.ua.ie > 0 && opt.value=="") {
               opt.value=""; //original set this to empty string, not sure why
            } 
...
XPE.trim=function(fm) {
 if (typeof(fm)=="string") {
  return  fm.replace(/^\s+|\s+$/g, "");
 }
 if (fm.elements) {
   var elm=fm.elements;
    for (var i=0; i<elm.length;++i) {
    elm[i].value=elm[i].value.replace(/^\s+|\s+$/g, "");
   }
 }
};
